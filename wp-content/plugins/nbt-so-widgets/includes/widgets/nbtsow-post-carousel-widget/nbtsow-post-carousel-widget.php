...
class NBTSOW_Post_Carousel_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Post carousel widget by Netbase Team', 'nbtsow' ),
			array( 'description' => __( 'A carousel to display post', 'nbtsow' ), 'panels_groups' => array('nbtsow-widgets'))
		);
		$this->skinlayout = array( 'default', 'sticky','fullwidth','boxed', 'home3');
	}

	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Post carousel', 'nbtsow' );
		if  ( $instance ) {	
		...
		if ( $nbt_query->have_posts() ) {
				...
				while( $nbt_query->have_posts() ) :
						...
						<div class="content-img-left col-sm-4 col-xs-12">
							<div class="thumbnail col-sm-6">
								...
								<?php 
								if ( has_post_thumbnail() ) { the_post_thumbnail('owl-pc-thumb'); } 
								?>
								</a>
							</div>
			...
			/*end_skin_sticky*/
				...
				echo '<div id="owl-pc-' . $instance['slider_alias'] . '" class="owl-carousel owl-post-carousel skinlayout-'.$skinlayout.'">';				
					...
					<div class="grid-item ">
						...
						<div class="home-blog-meta">								
							...
							?>
								...
								<?php 								
								echo '<span class="author">'.esc_html( get_the_author() ).'</span>'; ?>  </span>	
								<?php }
								if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				    ...
				    elseif ($skinlayout=='home3') {
				    	while ($nbt_query->have_posts()): $nbt_query->the_post();
				    ?>
						<div class="articles-inner-home3">
							...
							?>
						   	...
						   	<div class="articles-desc">
						   		<a href="<?php the_permalink(); ?>" class="articles-title"><?php echo wp_trim_words( get_the_title(), 9, null ); ?></a>					    
			...
			?>
				...
				jQuery(document).ready(function(){
					jQuery("#owl-pc-<?php echo $instance['slider_alias']; ?>").owlCarousel({
						margin: 30,
...
function NBTSOW_Post_Carousel_Widget() {
	register_widget( 'NBTSOW_Post_Carousel_Widget' );
}
add_action( 'widgets_init', 'NBTSOW_Post_Carousel_Widget' );
...
?>
<?php if(!empty( $link_addimg ) && !empty($linksc)){?>
...
<div class="nbt-image-banner-widget <?php if(!empty($nbtclass)) : echo $nbtclass; endif; ?>">
	<div class="nbt-box-img-item">
		<a href="<?php if($url) echo esc_url($url);  ?>"><img <?php foreach($attr as $n => $v) echo $n.'="' . esc_attr($v) . '" ' ?> /></a>
	</div>
	<div class="nbt-image-banner-info">
		...
		<?php if(empty( $link_addimg ) && empty($linksc) && !empty($url) && !empty($txt_btn)) { ?>

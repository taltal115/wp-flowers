...
<!-- START BODY -->
<body <?php body_class('nb_flower') ?>>

<h1 style="font-weight: 700;color: #444;text-transform: capitalize;background: transparent;">
    <?php echo esc_html($localized_table_text); ?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'nb_flower' ) ?></a><?php endif; ?>
</h1>

<?php do_action( 'yith_woocompare_before_main_table' ); ?>

<table class="compare-list" cellpadding="0" style="border: 1px solid #e1e1e1;" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    <thead>
    <tr>
        <th>&nbsp;</th>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'nb_flower' ) ?></td>
        </tr>
    ...
    <?php else : ?>
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                <td class="<?php echo esc_attr($product_class); ?>" style="background: transparent;">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->get_id() ) ) ?>" data-product_id="<?php echo esc_attr($product->id); ?>"><?php esc_html_e( 'Remove', 'nb_flower' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
        ...
        <?php foreach ( $fields as $field => $name ) : ?>
            ...
            <tr class="<?php echo esc_attr($field); ?>">
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id(); ?>
                    <td class="<?php echo esc_attr($product_class); ?>" style="background: transparent;"><?php
                        switch( $field ) {

                            case 'image':
                                echo '<div class="image-wrap">' . wp_get_attachment_image( $product->fields[$field], 'quick-view', 'style="border: 0;"' ) . '</div>';
                                break;
        ...
        <?php if ( $repeat_price == 'yes' && isset( $fields['price'] ) ) : ?>
            <tr class="price repeated">
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                    <td class="<?php echo esc_attr($product_class); ?>" style="background: transparent;"><?php printf('%s', $product->fields['price']); ?></td>
                <?php endforeach; ?>
        ...
        <?php if ( $repeat_add_to_cart == 'yes' && isset( $fields['add-to-cart'] ) ) : ?>
            <tr class="add-to-cart repeated">
                ...
                <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->get_id() ?>
                    <td class="<?php echo esc_attr($product_class); ?>" style="background: transparent;"><?php woocommerce_template_loop_add_to_cart(); ?></td>
                <?php endforeach; ?>
